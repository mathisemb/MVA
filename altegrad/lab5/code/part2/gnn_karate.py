...
features = np.eye(n) # we get accuracy = 1.0
#features = np.ones((n,1)) # we get accuracy = 0.2857 which is lower because it fails to distinguish the nodes
...
def train(epoch):
    ...
    model.train()
    optimizer.zero_grad()
    output, _ = model(features, adj)  #to make it work with the embedding
